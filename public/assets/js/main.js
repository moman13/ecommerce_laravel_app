...
// start slider-xs-navBar
$(window).scroll(function(){
    var $item = $('.top'),
			$itemTop = $item.position().top;
		if ($(window).scrollTop() >= $itemTop) {         
			$('header .xs-navBar').addClass('fixed-top');
			$('.top').addClass('top-xs-navBar');
		}
		else {
			$('header .xs-navBar').removeClass('fixed-top');
			$('.top').removeClass('top-xs-navBar');
	...
	$('#submit_login').on('click',function(e){
		...
		e.preventDefault();
		 ...
		 if(Email == '' || password == ''){
			$(".text-danger").html("Enter your username and password").show().fadeOut(5000);
		}else{
			$(".text-danger").html("Error in username or password").show().fadeOut(5000);
		}
	});
	...
	// 		$(".text-danger").html("").show().fadeOut(5000);
	// 	} else 
	// 	$(".text-danger").html("password does not match").show().fadeOut(3000);
	//   });
	$('#submit').on('click',function(e){
		...
		e.preventDefault();
		 ...
		 if(email_S == '' || password_S == ''|| username == ''|| confirm_password == ''){
			$(".text-danger").html("Enter your username and password").show().fadeOut(5000);
		}else{
			$(".text-danger").html("").show().fadeOut(5000);
		}
	...
	// $('#password_S, #confirm_password').on('keyup', function () {
	// 	if ($('#password').val() == $('#confirm_password').val()) {
	// 		$(".text-danger").html("").show().fadeOut(5000);
	// 	} else 
	// 	$(".text-danger").html("Password Matching").show().fadeOut(5000);
	//   });

	$('#password_S, #confirm_password').on('keyup', function () {
		if ($('#password_S').val() == $('#confirm_password').val()) {
		  $('.text-danger').html('Matching password').css('color', 'green').delay(3);
		} else 
		  $('.text-danger').html('Not Matching password').css('color', 'red');
	  });
/*
...
*/
	...
	$('.animate_body i').click(function(){
		
		$('html, body').animate({

			scrollTop: $('.xs-section-padding').offset().top+-10
		},600);
	});
